		...
		for (objectID, centroid) in objects.items():
			...
			# to determine direction
				...
				if not to.counted:
					...
					if direction < 0 and centroid[0] < (W//2) and centroid[0] > (W//2)-50:
						totalUp += 1
						response = requests.get('http://127.0.0.1:5000/param', params={"count": str(totalDown-totalUp)})
					...
					elif direction > 0 and centroid[0] > (W//2)-50 and centroid[0] < (W//2):
						totalDown += 1
						response = requests.get('http://127.0.0.1:5000/param', params={"count": str(totalDown-totalUp)})
